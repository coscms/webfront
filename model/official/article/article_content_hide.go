...
func init() {
	ContentHideDetectorRegister(`signIn`, echo.T(`登录后才能查看`), func(params *ContentHideParams) bool {
		return params.Customer == nil // 返回true表示需要隐藏内容，否则显示内容
	}, echo.T(`此处内容需要登录后方可阅读`))
	ContentHideDetectorRegister(`level`, echo.T(`只有特定等级的用户才能查看(多个等级ID用半角逗号“,”分隔，例如“[hide:level:1,2]隐藏内容[/hide]”)`), func(params *ContentHideParams) bool {
	...
	ContentHideDetectorRegister(`group`, echo.T(`只有特定用户组才能查看(多个组ID用半角逗号“,”分隔，例如“[hide:group:1,2]隐藏内容[/hide]”)`), func(params *ContentHideParams) bool {
		if params.Customer == nil {
			return true // 返回true表示需要隐藏内容，否则显示内容
		}
	...
	ContentHideDetectorRegister(`role`, echo.T(`只有特定角色才能查看(多个角色ID用半角逗号“,”分隔，例如“[hide:role:1,2]隐藏内容[/hide]”)`), func(params *ContentHideParams) bool {
		if params.Customer == nil {
			return true // 返回true表示需要隐藏内容，否则显示内容
		}
	...
	ContentHideDetectorRegister(`owner`, echo.T(`作者才能查看`), func(params *ContentHideParams) bool {
		if params.Customer == nil {
			return true
		}
	...
	}, echo.T(`此处内容只有本文作者才能查看`))
	ContentHideDetectorRegister(`paid`, echo.T(`需要付款后才能查看(需设置售价)`), func(params *ContentHideParams) bool {
