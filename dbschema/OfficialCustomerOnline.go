...
func (s Slice_OfficialCustomerOnline) Range(fn func(m factory.Model) error) error {
	for _, v := range s {
		if err := fn(v); err != nil {
			return err
		}
...
func (s Slice_OfficialCustomerOnline) RangeRaw(fn func(m *OfficialCustomerOnline) error) error {
	for _, v := range s {
		if err := fn(v); err != nil {
			return err
		}
...
func (s Slice_OfficialCustomerOnline) GroupBy(keyField string) map[string][]*OfficialCustomerOnline {
	r := map[string][]*OfficialCustomerOnline{}
	for _, row := range s {
		dmap := row.AsMap()
		vkey := fmt.Sprint(dmap[keyField])
		if _, y := r[vkey]; !y {
			r[vkey] = []*OfficialCustomerOnline{}
		}
...
func (s Slice_OfficialCustomerOnline) KeyBy(keyField string) map[string]*OfficialCustomerOnline {
	r := map[string]*OfficialCustomerOnline{}
	for _, row := range s {
		dmap := row.AsMap()
		vkey := fmt.Sprint(dmap[keyField])
...
func (s Slice_OfficialCustomerOnline) FromList(data interface{}) Slice_OfficialCustomerOnline {
	values, ok := data.([]*OfficialCustomerOnline)
	if !ok {
		for _, value := range data.([]interface{}) {
			row := &OfficialCustomerOnline{}
...
func (a *OfficialCustomerOnline) List(recv interface{}, mw func(db.Result) db.Result, page, size int, args ...interface{}) (func() int64, error) {
	...
	if err == nil {
		...
		case *[]*OfficialCustomerOnline:
			err = DBI.FireReaded(a, queryParam, Slice_OfficialCustomerOnline(*v))
		case []*OfficialCustomerOnline:
			err = DBI.FireReaded(a, queryParam, Slice_OfficialCustomerOnline(v))
		case factory.Ranger:
