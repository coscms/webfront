...
func (s Slice_OfficialCommonRemark) RangeRaw(fn func(m *OfficialCommonRemark) error) error {
	for _, v := range s {
		if err := fn(v); err != nil {
			return err
		}
...
func (s Slice_OfficialCommonRemark) GroupBy(keyField string) map[string][]*OfficialCommonRemark {
	r := map[string][]*OfficialCommonRemark{}
	for _, row := range s {
		dmap := row.AsMap()
		vkey := fmt.Sprint(dmap[keyField])
		if _, y := r[vkey]; !y {
			r[vkey] = []*OfficialCommonRemark{}
		}
...
func (s Slice_OfficialCommonRemark) KeyBy(keyField string) map[string]*OfficialCommonRemark {
	r := map[string]*OfficialCommonRemark{}
	for _, row := range s {
		dmap := row.AsMap()
		vkey := fmt.Sprint(dmap[keyField])
...
func (s Slice_OfficialCommonRemark) FromList(data interface{}) Slice_OfficialCommonRemark {
	values, ok := data.([]*OfficialCommonRemark)
	if !ok {
		for _, value := range data.([]interface{}) {
			row := &OfficialCommonRemark{}
...
func (a *OfficialCommonRemark) List(recv interface{}, mw func(db.Result) db.Result, page, size int, args ...interface{}) (func() int64, error) {
	...
	if err == nil {
		...
		case *[]*OfficialCommonRemark:
			err = DBI.FireReaded(a, queryParam, Slice_OfficialCommonRemark(*v))
		case []*OfficialCommonRemark:
			err = DBI.FireReaded(a, queryParam, Slice_OfficialCommonRemark(v))
		case factory.Ranger:
