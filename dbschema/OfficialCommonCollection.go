...
func (s Slice_OfficialCommonCollection) Range(fn func(m factory.Model) error) error {
	for _, v := range s {
		if err := fn(v); err != nil {
			return err
		}
...
func (s Slice_OfficialCommonCollection) RangeRaw(fn func(m *OfficialCommonCollection) error) error {
	for _, v := range s {
		if err := fn(v); err != nil {
			return err
		}
...
func (s Slice_OfficialCommonCollection) GroupBy(keyField string) map[string][]*OfficialCommonCollection {
	r := map[string][]*OfficialCommonCollection{}
	for _, row := range s {
		dmap := row.AsMap()
		vkey := fmt.Sprint(dmap[keyField])
		if _, y := r[vkey]; !y {
			r[vkey] = []*OfficialCommonCollection{}
		}
...
func (s Slice_OfficialCommonCollection) KeyBy(keyField string) map[string]*OfficialCommonCollection {
	r := map[string]*OfficialCommonCollection{}
	for _, row := range s {
		dmap := row.AsMap()
		vkey := fmt.Sprint(dmap[keyField])
...
func (s Slice_OfficialCommonCollection) FromList(data interface{}) Slice_OfficialCommonCollection {
	values, ok := data.([]*OfficialCommonCollection)
	if !ok {
		for _, value := range data.([]interface{}) {
			row := &OfficialCommonCollection{}
...
func (a *OfficialCommonCollection) List(recv interface{}, mw func(db.Result) db.Result, page, size int, args ...interface{}) (func() int64, error) {
	...
	if err == nil {
		...
		case *[]*OfficialCommonCollection:
			err = DBI.FireReaded(a, queryParam, Slice_OfficialCommonCollection(*v))
		case []*OfficialCommonCollection:
			err = DBI.FireReaded(a, queryParam, Slice_OfficialCommonCollection(v))
		case factory.Ranger:
